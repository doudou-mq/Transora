...
一个 OpenAI 兼容的假接口），把阶段 1 主链路跑一遍：注入 → 未配置引导 → 配置模型 →
全文对照（含 flex 容器场景）→ 三态切换 → 侧边栏 → 划词 → 恢复原文 → Popup → 各设置页。
...
## 阶段 1 未实现（按 `docs/00` 第九章取用规则，属后续阶段）

- 多模型对比（侧边栏多列并排 + 应用到页面）— 阶段 2
- 翻译历史（IndexedDB、搜索筛选、导出）— 阶段 2
- 动态内容 / SPA 自动补翻 — 阶段 3
- 快捷键自定义、混合式对比、虚拟滚动 — 阶段 3
- 图片 OCR / 语音朗读 / 生词本 / 账号体系 / 云端同步 / 流式输出 — **明确排除**
